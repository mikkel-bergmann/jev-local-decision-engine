...
- [ ] 1.1 [req: env-bootstrap] Create `requirements.txt` at the repo root pinning
...
- [ ] 1.2 [req: env-bootstrap] Create the environment with
...
- [ ] 2.1 [req: candidate-resolution] Add `tests/test_jev_engine.py` with a test that
...
- [ ] 2.2 [req: candidate-resolution] Create `jev_local_engine.py` with
...
- [ ] 3.1 [req: candidate-normalized-probabilities] In `tests/test_jev_engine.py`, add a test
...
- [ ] 3.2 [req: candidate-normalized-probabilities] In `jev_local_engine.py`, add
...
- [ ] 4.1 [req: mps-model-load, env-bootstrap] In `jev_local_engine.py`, add `load_model()`
...
- [ ] 5.1 [req: batched-field-scoring] In `tests/test_jev_engine.py`, add a test that builds
...
- [ ] 5.2 [req: batched-field-scoring] In `jev_local_engine.py`, add
...
- [ ] 5.3 [req: batched-field-scoring] In `jev_local_engine.py`, add `score_fields(...)` that
...
- [ ] 6.1 [req: schema-validated-output] In `tests/test_jev_engine.py`, add a test that
      constructing `IntentDecision` with `category="not_a_category"` raises a Pydantic
      `ValidationError`. Run it and observe it fail.
- [ ] 6.2 [req: schema-validated-output] In `jev_local_engine.py`, define `IntentDecision`
...
- [ ] 7.1 [req: batched-field-scoring] In `tests/test_jev_engine.py`, add a test that scores
...
- [ ] 8.1 [req: benchmark-report] In `jev_local_engine.py`, add a `__main__` block that loads
...
- [ ] 8.2 [req: *] Run `.venv/bin/python jev_local_engine.py`, confirm it prints valid JSON,
      and record the observed latency and resident memory against the sub-500 ms and 3-4 GB
      targets.
- [ ] 8.3 [req: candidate-normalized-probabilities] Run the engine on a calm positive message
